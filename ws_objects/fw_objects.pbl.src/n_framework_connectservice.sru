$PBExportHeader$n_framework_connectservice.sru
$PBExportComments$Generated Database Connection Service
forward
global type n_framework_connectservice from nonvisualobject
end type
end forward

global type n_framework_connectservice from nonvisualobject
end type
global n_framework_connectservice n_framework_connectservice
...
Choose Case is_connectfrom
	...
	Case IS_USE_INIFILE						/*  Populate Database Connection from INI file  */
		string ls_inifile = "D:\EDP Docs\PB--Framework\framework.ini"
		
		as_dbms			= ProfileString ( ls_inifile, "Database", "DBMS", "IN9 INFORMIX-9")
		as_database		= ProfileString ( ls_inifile, "Database", "Database", "db")
		as_userid		= ProfileString ( ls_inifile, "Database", "UserID", "mis")
		as_dbpass		= ProfileString ( ls_inifile, "Database", "DBPass", "00298277")
		as_logid			= ProfileString ( ls_inifile, "Database", "LogID", "")
		as_logpass		= ProfileString ( ls_inifile, "Database", "LogPassword",	"")
		as_server		= ProfileString ( ls_inifile, "Database", "Servername", "192.168.2.1@se_mal")
		as_dbparm		= ProfileString ( ls_inifile, "Database", "DBParm", "INET_DBPATH='/usr/database/asli/doc',INET_PROTOCOL='sesoctcp',INET_SERVICE='1526'")
	...
	Case IS_USE_REGISTRY						/*  Populate Database Connection from Registry  */
		...
		If RegistryGet ( ls_registrykey, "Servername", 	RegString!, as_server ) <> 1 Then
			RegistrySet ( ls_registrykey, "Servername", 	RegString!, "192.168.2.1@se_mal" )
			RegistryGet ( ls_registrykey, "Servername", 	RegString!, as_server )
		End If
		If RegistryGet ( ls_registrykey, "DBParm", 		RegString!, as_dbparm ) <> 1 Then
			RegistrySet ( ls_registrykey, "DBParm", 		RegString!, "INET_DBPATH='/usr/database/asli/doc',INET_PROTOCOL='sesoctcp',INET_SERVICE='1526'" )
	...
	Case IS_USE_SCRIPT							/*  Populate Database Connection from Script  */
		as_dbms			= "IN9 INFORMIX-9"
		as_database		= "db"
		as_userid		= "mis"
		as_dbpass		= "00298277"
		as_logid			= ""
		as_logpass		= ""
		as_server		= "192.168.2.1@se_mal"
		as_dbparm		= "INET_DBPATH='/usr/database/asli/doc',INET_PROTOCOL='sesoctcp',INET_SERVICE='1526'"
		as_lock			= ""
		as_autocommit	= "false"
...
on n_framework_connectservice.create
TriggerEvent( this, "constructor" )
end on

on n_framework_connectservice.destroy
